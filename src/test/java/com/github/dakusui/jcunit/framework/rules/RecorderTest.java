...
public class RecorderTest extends Recorder implements Serializable {
    ...
    @Test
    public void testInitializeDirSaveAndLoad() throws IOException {
        ...
        File testDataDir = testDataDirFor(baseDir.getAbsolutePath(), this.getId(), this.description);
    ...
    @Test
    public void givenRecorderAndReplayerAreSetFalse$whenInitializeDirSaveAndLoad$thenDoNotWriteAnything() throws IOException {
    ...
    @Test
    public void givenTestCaseTypeIsNotGenerated$whenInitializeDirSaveAndLoad$thenDoNotWriteAnything() throws IOException {
        System.setProperty(SystemProperties.KEY.RECORDER.key(), "true");
        System.setProperty(SystemProperties.KEY.REPLAYER.key(), "true");
    ...
    public void testDirectoryMethods() {
        wireMocks();
        assertEquals(
                new File(String.format("testbase/%s", this.getClass().getCanonicalName())),
                Recorder.testClassDataDirFor("testbase", this.getClass())
        );
        assertEquals(
                new File(String.format("testbase/%s/data-123", this.getClass().getCanonicalName())),
                Recorder.testCaseDataDirFor("testbase", 123, this.getClass())
        );
        assertEquals(
                new File(String.format("testbase/%s/data-123/methodName", this.getClass().getCanonicalName())),
    ...
    private void whenInitializeDirSaveAndLoad$thenDoNotWriteAnything(boolean checkGarbage) throws IOException {
        File baseDir = new File(this.getBaseDir());
        this.starting(this.description);
        ...
        if (checkGarbage) assertTrue(garbage.exists());
