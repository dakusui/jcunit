...
public class JCUnitTest extends DefaultRuleSetBuilder {
	...
	public abstract static class Base extends DefaultRuleSetBuilder {
		...
		public int in;
		public static int[] in() { return new int[]{123}; }
	...
	public static class MatchAndPass extends Base {
		...
		.incase(is(get("in"), 123), is(get("out"), 124));
	}
	...
	public static class CutOperatorMakesRuleSetIgnoreFollowingRules extends Base {
		...
		public RuleSet rules = 	ruleSet()
			...
			.expect(
				is(get("out"), 124)).cut()
			.incase(
				lt(get("in"), 999)) // Since 'in' is less than 999, this rule matches, too.
			.expect(
				is(get("out"), 999)) // This must fail if this rule matches.
		;
	}
	...
	public static class MatchOneOfRulesAndPass_1 extends Base {
		...
		public RuleSet rules = 	ruleSet()
			...
			.expect(
				is(get("out"), 124))
			.incase(
				gt(get("in"), 999)) // since 'in' is less than 999, this DOESNT match
			.expect(
				is(get("out"), 124))
		;
	}
	...
	public static class MatchOneOfRulesAndPass_2 extends Base {
		@Rule
		public RuleSet rules = 	ruleSet()
			// The order of execution is different from the other's.
			.incase(
				gt(get("in"), 999)) // since 'in' is less than 999, this DOESNT match
			.expect(
				is(get("out"), 124))
	...
	public static class MatchNoneOfNestedAndFail extends Base {
		...
		.expect(ruleSet()
			.incase(
				not(is(get("in"), 123))) // since 'in' is 123, this doesn't match.
			.expect(
				is(get("out"), 124))
			.incase(
				gt(get("in"), 999)) // since 'in' is 123, this does neither match.
			.expect(
				is(get("out"), 124))
			);
	}
	...
	public static class MatchAllNestedAndPass extends Base {
		...
		.expect(ruleSet()
			...
			.expect(
				is(get("out"), 124))
			.incase(
				lt(get("in"), 999)) // since 'in' is less than 999, this also matches.
			.expect(
				is(get("out"), 124))
			.otherwise(
				is(get("out"), 999))		
	...
	public static class MatchButFail extends Base {
		...
		ruleSet().incase(is(get("in"), 123), is(get("out"), 456));
	}
	
	public static class NoMatch extends Base {
		@Rule
		public RuleSet rules = 
		ruleSet().incase(is(get("in"), 456), is(get("out"), 124));
