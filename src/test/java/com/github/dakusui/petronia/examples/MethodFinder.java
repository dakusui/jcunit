...
public class MethodFinder {
	...
	private Class<?> parseSignature(int arrayLevel, Reader r) throws ClassNotFoundException {
		int c = 0;
		try {
			c = r.read();
			if (c < 0) return null; 
			if (c == 'Z') return getClass(arrayLevel, Boolean.TYPE);
			if (c == 'B') return getClass(arrayLevel, Byte.TYPE);
			if (c == 'C') return getClass(arrayLevel, Character.TYPE);
			if (c == 'S') return getClass(arrayLevel, Short.TYPE);
			if (c == 'I') return getClass(arrayLevel, Integer.TYPE);
			if (c == 'J') return getClass(arrayLevel, Long.TYPE);
			if (c == 'F') return getClass(arrayLevel, Float.TYPE);
			if (c == 'D') return getClass(arrayLevel, Double.TYPE);
			if (c == 'L') return getClass(arrayLevel, readClassNameAndLoadTheClass(r));
			if (c == '[') return parseSignature(arrayLevel + 1, r);
		} catch (IOException e) {
			// since the underlying data source is a string,
			// IO exception will never happen.
		}
		throw new IllegalArgumentException(String.format("'%s': non-recognizable character.", c));
	}

	private Class<?> getClass(int arrayLevel, Class<?> type) {
		if (arrayLevel == 0) return type;
	...
	private Class<?> readClassNameAndLoadTheClass(Reader r) throws ClassNotFoundException {
		StringBuilder builder = new StringBuilder(20);
		int c;
		String className = null;
		try {
			while ((c = r.read()) != -1) {
				if (c == ';') return Class.forName(className = builder.toString());
				builder.append((char)c);
			}
		} catch (ClassNotFoundException e) {
			String msg = String.format("Class '%s' isn't a valid class name.", className);
			throw new IllegalArgumentException(msg, e);
		} catch (IOException e) {
			// Since the underlying data source is a string,
			// IOException will never be thrown.
		}
		String msg = String.format("Unterminated class name is found (%s)", builder.toString());
