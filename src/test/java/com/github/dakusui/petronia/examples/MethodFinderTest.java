...
public class MethodFinderTest extends DefaultRuleSetBuilder {
	...
	public static class MethodFinderTest1 extends MethodFinderTest {
		...
		.incase(
				and(
						isoneof(get("clazz"), String.class, Object.class), 
						is(get("methodName"), "toString"),
						eq(get("parameterTypes"), new Object[0])
				),
				not(is(get("method"), null))
		).incase(
				and(
						is(get("methodName"), "equals"),
						eq(get("parameterTypes"), new Object[0])
				),
				isinstanceof(get("exception"), NoSuchMethodException.class)
		).incase(
				and(
						is(get("methodName"), "matches"),
						eq(get("parameterTypes"), new Object[0])
				),
				isinstanceof(get("exception"), NoSuchMethodException.class)
		).incase(
				is(get("clazz"), null),
				isinstanceof(get("exception"), NullPointerException.class)
		).incase(
		...
		public void test() throws SecurityException, NoSuchMethodException {
			System.err.printf("clazz=%s, methodName=%s, paramTypes=%s\n", this.clazz, this.methodName, ArrayUtils.toString(this.parameterTypes));
			try {
	...
	public static class MethodFinderTest2 extends MethodFinderTest {
		...
		.incase(
				...
				isoneof(get("parameterTypes"), "Q", "Lnot.Found;"),
				isinstanceof(get("exception"), IllegalArgumentException.class)
		).incase(
				is(get("methodName"), "notFound"),
				isinstanceof(get("exception"), NoSuchMethodException.class)
		).incase(
				and(
					...
					not(
							or(
									eq(get("parameterTypes"), "Lnot.Found;"),
									eq(get("parameterTypes"), ""),
	...
	public static String[] methodName() {
		return new String[]{
			"toString",	"equals", "matches", "notFound", null
		};
	}
	...
	public static void main(String[] args) throws ClassNotFoundException, SecurityException, NoSuchMethodException {
		...
		System.out.println(new MethodFinder().setClass(String.class).setName("toString").find());
		System.out.println(new MethodFinder().setClass(String.class).setName("charAt").setSignature("I").find());
		System.out.println(new MethodFinder().setClass(String.class).setName("equals").setSignature(new Class[]{Object.class}).find());
		System.out.println(new MethodFinder().setClass(String.class).setName("equals").setSignature(new Class[]{null}).find());
		System.out.println(new MethodFinder().setClass(String.class).setName("charAt").find());
	}
}
