...
public class JCUnit extends Parameterized {
  ...
  private static void validateConstraintChecker(ConstraintChecker constraintChecker) {
    ...
    for (String eachTag : constraintChecker.getTags()) {
      ...
      CompatUtils.checkplugin(eachTag != null, "Constraint checker must not return null as a tag. (%s)", fqcn);
      CompatUtils.checkplugin(eachTag.matches(regex), "A tag returned by constraint checker (%s) must match %s but not (%s)", eachTag, regex, fqcn);
    }
  }
  ...
  public static class Engine implements TestSuite.Typed.ModelingEngine {
    ...
    public List<NumberedTestCase> generateTestCases(Config config, List<FrameworkMethod> preconditionMethods, List<FrameworkMethod> customTestCaseMethods, ConstraintChecker constraintChecker, final FactorSpace factorSpace, CoveringArrayEngine coveringArrayEngine) {
      ...
      registerTestCases(
          ...
          preconditionMethods);
      CompatUtils.checkenv(testCases.size() > 0, "No test to be run was found.");
      return testCases;
    }
