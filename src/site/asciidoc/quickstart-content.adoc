...
image:JCunit-logo-compact.png[] is a model based testing tool powered by combinatorial testing techniques.

Current version of JCUnit (1.0.x) is not compatible with older versions anymore.
Users who need to use older ones, please refer to https://github.com/dakusui/jcunit/wiki/0.8.x[JCUnit-0.8.x].

In this document, the new version is called *JCUnitX* to tell it from older ones when necessary.

== About 1.0.x

1.0.x and later is actively being developed right now and called *JCUnitX*.
It requires Java SE8.

Due to changes introduced in 1.0.x, *JCUnitX*, it is not compatible with the older versions anymore, although the programming style is the same.

=== Installation and how to use it.

Please use following dependencies to use *JCUnitX*.
...
Replace `$\{jcunit-version\}` in the fragment with `{jcunit-version}`.
...
* JUnit5 support
* Introduction of new "joiner" algorithm: "Weaken-product combinatorial join"
* Internal mechanism clean-up
* Removal of FSM support (because REGEX is more useful)
* Improvements in REGEX support
...
example of the new annotation style link:/src/java/test/com/github/dakusui/jcunit8/examples/BankAccountExample.java[here].

These days, people more and more tend to prefer http://junit.org/junit4/javadoc/4.12/org/junit/experimental/theories/Theories.html[`@Theories` style],
...
To negate a condition, you can do
...
== The Test Suite Generation Pipeline
...
image::ThePipeline/Slide1.jpg[align="center"]
...
image::ThePipeline/Slide2.jpg[align="center"]
.Engine
...
`Config.Factory` interface, it might be good idea to use it as a value for `@ConfigureWith` annotation when it is absent.
* IPO-G+ performance improvements.
* FSM feature has already been implemented, but not yet tested in 0.8.x line.
* Make the pipeline execute concurrently its internal processes where possible.

== References

- [[JCUnitWiki]] JCUnit Wiki, https://github.com/dakusui/jcunit/wiki
- [[JCUnitBlog]] JCUnit blog; Japanese; 日本語, http://jcunit.hatenablog.jp/
- [[TestDesignAsCode]] "Test Design as Code" (A paper on this product; Presented at IEEE International Conference on Software Testing 2017), http://ieeexplore.ieee.org/document/7928008/
