...
public class BestPairwiseTestArrayGenerator<T, U> extends PairwiseTestArrayGenerator<T, U>{
	private static final Logger LOGGER = LoggerFactory.getLogger(BestPairwiseTestArrayGenerator.class);
	@Override
	protected TestRunSet composeTestRunSet(Map<Integer, T> indexToKeyMap) {
		if (this.domains.size() == 0) {
			LOGGER.warn("The domains has no member, falling back to PairwiseTestArrayGenerator.");
		...
		TestRunSet ret = null;
		////
		// Id of domain key list, this should be the only information to be stored in
