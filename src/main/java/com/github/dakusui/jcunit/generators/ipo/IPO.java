...
public class IPO {
	 ...
	 *             the runs in T that cover the maximum number of pairs between
	 *             parameter Fi, 
	 * 
	 */
	...
	private TestRunSet vg(TestRunSet T, Set<ValuePair> MP) {
		...
		for (ValuePair pair: MP) {
			...
			List<TestRun> runsWhoseAis_rAndBisDC = lookUp(lookUp(T$, pair.A, pair.r), pair.B, DC);
			...
			List<TestRun> runsWhoseBis_sAndAisDC = lookUp(lookUp(T$, pair.A, DC), pair.B, pair.s);
