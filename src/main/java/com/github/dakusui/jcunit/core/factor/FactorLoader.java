...
public class FactorLoader {
  ...
  ValidationResult validate(Field f) {
    ...
    LevelsFactory<?> levelsFactory = NullLevelsFactory.INSTANCE;
    ...
    if (overridingLevelsMethods.length > 1) {
      errors.add(String.format(
          ...
          Utils.join(",", new Utils.Formatter<Method>() {
            @Override public String format(Method m) {
              return m.getName();
            }
          })
      ));
      ...
      if (methodNameMappings.containsKey(fieldType)) {
        ...
        // method was found).
        errors.add(String.format(
            "For the field '%s'(%s), 'levelsFactory' needs to be provided.",
            f, fieldType));
      }
    }
