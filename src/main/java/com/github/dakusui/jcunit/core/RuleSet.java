...
public class RuleSet implements TestRule {
  ...
  public Statement apply(final Statement base, final Description desc) {
    ...
    return new Statement() {
      ...
      public void evaluate() throws Throwable {
        ...
        try {
          ...
          if (throwable == null) {
            ...
            try {
              ok = verify(desc, target, RuleSet.this);
            ...
            // //
            // Handling a throwable which isn't caught by jcunit framework.
            // Typically thrown for bugs in jcunit itself.
      ...
      /* NOT NECESSARY FOR NEW REPORTING MECHANISM */
  ...
  public boolean isLeaf(int objectId) {
  ...
  /**
   * Adds a pair of a 'condition' and 'expectation' which should be satisfied if
   * the 'condition' is evaluated <code>true</code> to this object. Pairs are
   * evaluated in the order where they are added. And <code>expect</code> can be
   * another <code>RuleSet</code> object, which will be evaluated in a nested
  ...
  private boolean evalp(Pair pair, Object p, String testName)
    ...
    try {
      ...
      if (p == pair.cond) {
        pair.condResult = ret;
      } else if (p == pair.nested) {
        pair.nestedResult = ret;
      } else {
  ...
  public boolean apply(Report report, String testName) throws JCUnitException,
    ...
    for (Pair cur : rulePairs) {
      // //
      // reset the indentation level.
      report.indent = indentLevel;
      if (report.check(testName, cur.cond, evalp(cur, cur.cond, testName))) {
