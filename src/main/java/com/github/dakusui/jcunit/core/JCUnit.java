...
public class JCUnit extends Suite {
  ...
  public JCUnit(Class<?> klass) throws Throwable {
    ...
    try {
      ...
      id = registerLabeledTestCases(id, filter, violations,
          TestCaseType.Custom);
      if (hasParametersMethod()) {
        registerLabeledTestCases(id, filter, allCustomTestCases(),
  ...
  private int registerLabeledTestCases(int id, TestCaseFilter filter,
      Iterable<LabeledTestCase> violations, TestCaseType testCaseType)
      throws InitializationError {
    for (LabeledTestCase labeledTestCases : violations) {
      if (filter.shouldBeExecuted(id)) {
        runners.add(new JCUnitRunner(getTestClass().getJavaClass(),
            id, testCaseType, labeledTestCases.getLabels(),
            labeledTestCases.getTestCase()));
      }
      id++;
    }
    return id;
