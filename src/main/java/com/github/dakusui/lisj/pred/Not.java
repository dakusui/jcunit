...
public class Not extends LogicalPredicate {
	...
	protected FormResult evaluateLast(Context context, Object[] evaluatedParams, FormResult lastResult) {
		...
		if (!(target instanceof Boolean)) {
			throw new IllegalArgumentException(msgReturnedTypeMismatch(Boolean.class, target));
		}
	...
	protected FormEvaluator newEvaluator(Context context, Object params) {
		/*
		 * Since it's sure this class doesn't need to 
		 * create child context, FormEvaluator is created with incoming context. 
		 */
		return new FormEvaluator(context, this, checkParams(params), new FormResult(0, Basic.length(params), null));
	}
