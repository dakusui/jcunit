...
public class Basic {
	...
	public static class ConsIterator extends SexpIterator {
			...
			if (atom(target)) throw new IllegalArgumentException();
			this.target = target;
		}
		...
		public boolean hasNext() {
			if (cur < ((Object[])target).length) return true;
			return false;
		}
		...
		public Object next() {
			...
			if (cur >= len) throw new NoSuchElementException();
			if (cur == len - 1)	 {
				Object last;
	...
	public static SexpIterator iterator(final Object obj) {
		if (atom(obj) || length(obj) == 1) return new SexpIterator() {
	...
	public static Object get(Object obj, int index) {
		if (atom(obj)) throw new IllegalArgumentException();
		int len = ((Object[])obj).length;
		if (index < len - 1) return ((Object[])obj)[index];
		else if (index == len - 1) {
	 ...
	 * concise string than the one java.lang.Object#toString method returns,
	 * i.e. package names will be omitted.
