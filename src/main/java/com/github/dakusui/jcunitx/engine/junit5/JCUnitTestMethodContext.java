...
import com.github.dakusui.jcunitx.engine.junit5.compat.*;
...
import static com.github.dakusui.jcunitx.engine.junit5.JCUnitTestMethodContext.ResolverType.AGGREGATOR;
...
public class JCUnitTestMethodContext implements TestMethod {
  ...
  public JCUnitTestMethodContext(Method testMethod) {
    Parameter[] parameters = testMethod.getParameters();
    this.resolverTypes = new ArrayList<>(parameters.length);
    this.resolvers = new Resolver[parameters.length];
    for (Parameter parameter : parameters) {
      this.resolverTypes.add(isAggregator(parameter) ? AGGREGATOR : ResolverType.CONVERTER);
    }
  }
  ...
  static class Converter implements Resolver {
    ...
    private static final Converter DEFAULT = new Converter(ArgumentConverter.DEFAULT_INSTANCE);
  ...
  static class Aggregator implements Resolver {
    ...
    @Override
    public Object resolve(ParameterContext parameterContext, Object[] arguments) {
      ArgumentsAccessor accessor = new ArgumentsAccessor.Default(arguments);
      try {
